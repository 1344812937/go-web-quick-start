  ...
  <div class="page-stack settings-page">
    ...
    <el-form v-else label-position="top" class="settings-form" @submit.prevent="saveSettings">
      ...
      <section class="surface-panel settings-section">
        ...
        <div class="settings-fields settings-grid">
          <el-form-item label="管理会话时长（小时）">
            <div class="session-ttl-control">
              <el-input-number v-model="form.gatewayConfig.sessionTTLHours" :min="0" :max="8760" controls-position="right" />
              <small>填 0 表示永久登录；保存后立即作用于现有会话。</small>
            </div>
          </el-form-item>
...
.session-ttl-control { display: grid; justify-items: start; gap: 8px; }
.session-ttl-control small { color: var(--rose-text-muted); font-size: 11px; line-height: 1.6; }
