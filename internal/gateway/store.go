...
func (s *Store) cleanupExpired() {
	...
	if s.configManager != nil {
		sessionTTL = config.EffectiveSessionTTL(s.configManager.GetConfig())
	}
	if sessionTTL > 0 {
		// Session expiry follows the current setting, so changing the setting
		// also changes how existing rows are cleaned up.
		_ = s.db.Where("created_at < ?", now.Add(-sessionTTL)).Delete(&AdminSession{}).Error
	}
