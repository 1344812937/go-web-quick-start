...
func TestOpenBrowserOnStart(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Setenv(StartOpenWebKey, test.value)
			if test.unset {
				if err := os.Unsetenv(StartOpenWebKey); err != nil {
					t.Fatalf("os.Unsetenv() error = %v", err)
				}
			}
...
func TestResolveStartOpenWebPrecedence(t *testing.T) {
	tests := []struct {
		name       string
		args       []string
		envFile    string
		...
		{name: "default", unsetEnv: true, want: true},
		{name: "process environment", processEnv: "false", want: false},
		{name: "env file overrides process environment", envFile: "START_OPEN_WEB=true\n", processEnv: "false", want: true},
		{name: "command line overrides env file", args: []string{"--START_OPEN_WEB=false"}, envFile: "START_OPEN_WEB=true\n", processEnv: "true", want: false},
		{name: "short command line form", args: []string{"-START_OPEN_WEB=false"}, envFile: "START_OPEN_WEB=true\n", processEnv: "true", want: false},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.envFile != "" {
				if err := os.WriteFile(envPath, []byte(test.envFile), 0o600); err != nil {
					t.Fatal(err)
				}
			}
			t.Setenv(StartOpenWebKey, test.processEnv)
			if test.unsetEnv {
				if err := os.Unsetenv(StartOpenWebKey); err != nil {
					t.Fatal(err)
				}
			...
			got, err := resolveStartOpenWeb(test.args, envPath)
			if err != nil {
				t.Fatalf("resolveStartOpenWeb() error = %v", err)
			}
			if got != test.want {
				t.Fatalf("resolveStartOpenWeb() = %t, want %t", got, test.want)
			}
		})
	}
}

func TestResolveStartOpenWebValidatesHighestPriorityValue(t *testing.T) {
	envPath := filepath.Join(t.TempDir(), ".env")
	if err := os.WriteFile(envPath, []byte("START_OPEN_WEB=invalid\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	t.Setenv(StartOpenWebKey, "false")

	if _, err := resolveStartOpenWeb(nil, envPath); err == nil {
		t.Fatal("resolveStartOpenWeb() error = nil, want invalid .env error")
	}
	got, err := resolveStartOpenWeb([]string{"--START_OPEN_WEB=true"}, envPath)
	if err != nil {
		t.Fatalf("command line override returned error: %v", err)
	}
	if !got {
		t.Fatal("command line override = false, want true")
	}
}

func TestAllowIframeEmbedding(t *testing.T) {
	tests := []struct {
		name      string
		value     string
		unset     bool
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Setenv(AllowIframeEnvKey, test.value)
			if test.unset {
				if err := os.Unsetenv(AllowIframeEnvKey); err != nil {
					t.Fatalf("os.Unsetenv(%s) error = %v", AllowIframeEnvKey, err)
				}
			}
			got, err := AllowIframeEmbedding()
			if test.wantError {
				if err == nil {
					t.Fatal("AllowIframeEmbedding() error = nil, want an error")
				}
				return
			}
			if err != nil {
				t.Fatalf("AllowIframeEmbedding() error = %v", err)
			}
			if got != test.want {
				t.Fatalf("AllowIframeEmbedding() = %t, want %t", got, test.want)
			}
		})
	}
}
