...
func (acm *ApplicationConfigManager) Load() {
	...
	if err := normalizeSessionTTLHours(&cfg.GatewayConfig); err != nil {
		panic(fmt.Sprintf("解析管理会话配置失败: %v", err))
	}
...
type GatewayConfig struct {
	...
	SessionTTLHours                int      `toml:"session_ttl_hours" json:"sessionTTLHours" default:"0"`
...
func (acm *ApplicationConfigManager) Save(cfg *ApplicationConfig) error {
	...
	configCopy := *cfg
	if err := normalizeSessionTTLHours(&configCopy.GatewayConfig); err != nil {
		return err
	}
...
// EffectiveSessionTTL returns the currently configured administrator session lifetime.
// A zero duration means the session has no server-side expiration.
func EffectiveSessionTTL(cfg *ApplicationConfig) time.Duration {
	if cfg == nil || cfg.GatewayConfig.SessionTTLHours <= 0 {
		return 0
	}
	return time.Duration(cfg.GatewayConfig.SessionTTLHours) * time.Hour
}

func normalizeSessionTTLHours(gatewayConfig *GatewayConfig) error {
	if gatewayConfig.SessionTTLHours < 0 {
		return fmt.Errorf("管理会话时长不能为负数，0 表示永久")
	}
	if gatewayConfig.SessionTTLHours > MaxSessionTTLHours {
		return fmt.Errorf("管理会话时长不能超过 %d 小时", MaxSessionTTLHours)
	}
	return nil
}
