...
func TestGatewayConfigDefaultsPayloadLogDetail(t *testing.T) {
	...
	if gatewayConfig.SessionTTLHours != DefaultSessionTTLHours {
		t.Fatalf("session TTL = %d, want %d (permanent)", gatewayConfig.SessionTTLHours, DefaultSessionTTLHours)
	}
}

func TestEffectiveSessionTTL(t *testing.T) {
	if got := EffectiveSessionTTL(nil); got != 0 {
		t.Fatalf("nil session TTL = %s, want permanent", got)
	}
	if got := EffectiveSessionTTL(&ApplicationConfig{}); got != 0 {
		t.Fatalf("default session TTL = %s, want permanent", got)
	}
	if got := EffectiveSessionTTL(&ApplicationConfig{GatewayConfig: GatewayConfig{SessionTTLHours: 2}}); got != 2*time.Hour {
		t.Fatalf("configured session TTL = %s, want 2h", got)
	}
}

func TestApplicationConfigManagerSaveValidatesSessionTTL(t *testing.T) {
	originalConfigPath := configPath
	configPath = filepath.Join(t.TempDir(), "config.toml")
	t.Cleanup(func() { configPath = originalConfigPath })

	manager := &ApplicationConfigManager{}
	cfg := &ApplicationConfig{GatewayConfig: GatewayConfig{SessionTTLHours: 24}}
	if err := manager.Save(cfg); err != nil {
		t.Fatalf("Save(24h) error = %v", err)
	}
	if got := manager.GetConfig().GatewayConfig.SessionTTLHours; got != 24 {
		t.Fatalf("runtime session TTL = %d, want 24", got)
	}

	cfg.GatewayConfig.SessionTTLHours = -1
	if err := manager.Save(cfg); err == nil {
		t.Fatal("Save(negative TTL) error = nil")
	}
	if got := manager.GetConfig().GatewayConfig.SessionTTLHours; got != 24 {
		t.Fatalf("invalid save changed runtime session TTL to %d", got)
	}
}
...
func TestApplyRuntimeFallbacksUsesDefaultWebAddress(t *testing.T) {
	...
	changed, err := applyRuntimeFallbacks(cfg)
	if err != nil {
