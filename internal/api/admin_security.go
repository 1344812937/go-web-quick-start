...
const (
	adminSessionCookie        = "gateway_admin_session"
	permanentSessionCookieAge = 10 * 365 * 24 * time.Hour
)
...
func (s *AdminSecurity) setSessionCookie(c *gin.Context, rawToken string) {
	cfg := s.currentConfig()
	ttl := config.EffectiveSessionTTL(cfg)
	secure := false
	if cfg != nil {
...
func (s *AdminSecurity) refreshSessionCookie(c *gin.Context) {
	rawToken, err := c.Cookie(adminSessionCookie)
	if err == nil && rawToken != "" {
		s.setSessionCookie(c, rawToken)
	}
...
func (s *AdminSecurity) clearSessionCookie(c *gin.Context) {
	...
	c.Writer.Header().Set("Set-Cookie", (&http.Cookie{
		Name:     adminSessionCookie,
		Path:     "/api",
		MaxAge:   -1,
		HttpOnly: true,
...
func (s *AdminSecurity) currentConfig() *config.ApplicationConfig {
