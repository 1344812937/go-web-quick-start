...
服务优先监听 `0.0.0.0:8888`，管理控制台地址为 `http://127.0.0.1:8888/static/`；若 8888 已被占用，会自动切换到其他可用端口并在日志中输出实际地址。`START_OPEN_WEB` 控制启动后是否自动打开浏览器，默认值为 `true`，可通过启动参数、当前运行目录的 `.env` 或进程环境变量配置；同时配置时优先级依次为启动参数、`.env`、进程环境变量。例如 `go run . --START_OPEN_WEB=false`。启动时缺失的必要配置会直接使用默认值或自动生成随机值；若关键环境变量缺失，程序会在当前目录创建或补全权限为 `0600` 的 `.env`，生成随机主密钥和管理员登录凭据，请在登录前查看并妥善保管这些值。
...
| `START_OPEN_WEB` | 可选，默认 `true` | 控制启动后是否打开本地管理页面；启动参数使用 `--START_OPEN_WEB=<true\|false>` |
| `GATEWAY_ALLOW_IFRAME` | 可选，默认 `true` | 允许其他页面通过 iframe 嵌入；设为 `false` 时返回 `X-Frame-Options: DENY` |
